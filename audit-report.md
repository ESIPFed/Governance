...
| A-04 | review | `Bylaws/readme.md` | Header states "Restated as of December 02, 2016" with no subsequent amendment dates | Confirm whether the amendment history is complete before migration; governance committee to establish dating practice going forward. |
| A-05 | fix | All readme files | All navigation links are hardcoded GitHub blob URLs (111 total: 74 in `Bylaws/readme.md`, 37 in `ESIP Policies and Procedures/readme.md`) — will break on repo rename/fork | Replaced by Quarto sidebar navigation during migration |
...
| A-09 | review | `README.md`, `Standing Committee and Cluster Policies and Procedures/Funding Friday Rules.md`, `ESIP Policies and Procedures/1.0 Corporate/ESIP P&P 1.1 Goals.md` | Multiple files contain hyperlinks to `wiki.esipfed.org` — external wiki that may no longer be maintained (prose-only mentions in other files such as `1.9 Data Privacy Policy.md` are out of scope for this finding) | Review whether wiki hyperlinks are still valid; update or remove stale links during migration |
...
**Step 2 — Files missing from index:** The `comm` command matched against `blob/master` URLs, so the BenGalewsky fork link (1.10 MoU) also appeared as unmatched — but `1.10 Memoranda of Understanding.md` is linked from the index (just via a fork URL), so it is not truly orphaned. Three files are genuinely absent from the index: `1.0 Definitions.md`, `3.3A FiCom annual budget cycle.md`, and `3.3F ESIP Committee Budget Request Policy.md`.
...
**Step 5 — Hardcoded GitHub blob URLs:** The `grep -rn "github.com.*blob/master"` command returned 0 because the macOS BSD `grep` handles the dot-star pattern differently in recursive mode; direct per-file greps (`grep -c "github.com.*blob/master"`) confirm 37 links in the P&P readme and 74 in the Bylaws readme, for **111 total** hardcoded GitHub blob/master links across the two index files. This exceeds the expected >50 and confirms A-05.
